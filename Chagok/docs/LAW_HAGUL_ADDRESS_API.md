...
- **대상 서비스**
  ...
  - 국가법령정보 공동활용 개발자 LAB > 법령 한글주소 안내:contentReference[oaicite:0]{index=0}
- **Base URL**
...
| 삼단비교(2) | `/법령/3단비교/{title}`                        | 동일 기능, 다른 Prefix 방식([법률 열린마당][1])      |
...
| 유형    | 패턴                                        | 매핑                                     |
| ----- | ----------------------------------------- | -------------------------------------- |
| 기본    | `/영문법령/{title}`                           | `title` = 법령명(한글 기준)([법률 열린마당][1])     |
...
| 번호+발효일(단독)    | `/조약/({identifier},{date_val})`         | 제목 없이 번호·발효일만으로 조회 가능([법률 열린마당][1]) |
...
| 시행일 포함* | `/법령별표서식/({title},{enforce_date},{identifier})` | 구현 시 선택적 지원 (공식 안내는 2항목형 위주)([법률 열린마당][1]) |
...
여러 중앙부처가 **단일 Endpoint**를 공유하며, 부처명은 URL Path에 노출되지 않는다.([법률 열린마당][1])
...
3. **괄호 파라미터 정렬 규칙**
   ...
   * **법령**: `(시행일자?, 공포번호?, 공포일자?)`
   * **행정규칙/자치법규**: `(발령번호 or 공포번호, 발령/공포일자)`
   * **판례·재결례**: `(사건번호, 의결/판결일자?)`
...
> 아래 예시는 **핵심 로직 골격**만 제시한다. 실제 구현에서는 카테고리별 세부 패턴을 분기·확장하면 된다.
...
> 실제 프로젝트에서는 위 수도코드를 **카테고리 확장 가능한 Strategy 패턴**으로 쪼개거나, `law_url_patterns` 테이블을 두고 **데이터 기반 라우팅**으로 구현하는 것이 유지보수 측면에서 좋다.
...
AI 에이전트와 백엔드 개발자가 곧바로 **DB 스키마 설계·URL 생성 로직 구현**에 사용할 수 있도록 구조화한 기술 사양이다.([법률 열린마당][1])


[1]: https://open.law.go.kr/LSO/lab/hangulAddr.do "국가법령정보 공동활용"
